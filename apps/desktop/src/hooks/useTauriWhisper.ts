...
export function useTauriWhisper() {
  ...
  const updateDiarizationThreshold = useCallback((val: number) => {
    ...
    try {
      ...
      invoke('plugin:whisper-core|set_diarization_threshold', { threshold: val }).catch(console.error);
    } catch {
      // ignore
    }
  }, []);
  ...
  const renameSpeaker = useCallback(async (id: string, newName: string) => {
    ...
    if (id.startsWith('speaker_')) {
      ...
      if (!isNaN(num)) {
        try {
          await invoke('plugin:whisper-core|rename_voice_profile', { speakerId: num, name: trimmed });
        } catch (err) {
  ...
  const deleteSpeaker = useCallback(async (id: string) => {
    try {
      ...
      if (id.startsWith('speaker_')) {
        const num = parseInt(id.replace('speaker_', ''), 10);
        if (!isNaN(num)) {
          await invoke('plugin:whisper-core|delete_voice_profile', { speakerId: num });
        }
      }
  ...
  const clearVoiceProfiles = useCallback(async () => {
    try {
      await invoke('plugin:whisper-core|clear_voice_profiles');
      setSpeakerMap({});
  ...
  const getVoiceProfiles = useCallback(async () => {
    try {
      const profiles = await invoke<{ id: number; name?: string | null; sample_count: number; is_verified: boolean }[]>('plugin:whisper-core|get_voice_profiles');
  ...
  const mergeSpeakers = useCallback(async (sourceSpeakerId: string, targetSpeakerId: string) => {
    ...
    try {
      await invoke('plugin:whisper-core|merge_speakers', {
  ...
  const discoverModels = useCallback(async () => {
    try {
      const info = await invoke<DownloadedModelsInfo>('plugin:whisper-core|get_downloaded_models');
      setModelsInfo(info);
  ...
  const refreshCapturableApps = useCallback(async () => {
    try {
      const apps = await invoke<CapturableApp[]>('plugin:whisper-core|list_capturable_apps');
  ...
  const requestScreenCapturePermission = useCallback(async () => {
    try {
      const granted = await invoke<boolean>('plugin:whisper-core|request_screen_capture_permission');
      if (granted) {
  ...
  const downloadModel = useCallback(async (target: 'whisper' | 'qwen' | 'vad' | 'diarization', modelId?: string) => {
    ...
    try {
      if (target === 'whisper') {
        ...
        await invoke('plugin:whisper-core|download_model', { modelId: id });
      } else if (target === 'vad') {
        await invoke('plugin:whisper-core|download_silero_vad_model');
      } else if (target === 'diarization') {
        await invoke('plugin:whisper-core|download_diarization_models');
      }
  ...
  const loadModels = useCallback(async () => {
    ...
    try {
      ...
      if (backend === 'whisper') {
        ...
        await invoke('plugin:whisper-core|load_model', {
      ...
      if (vadEnabled && modelsInfo?.vad_model_path) {
        await invoke('plugin:whisper-core|load_vad_model', {
          modelPath: modelsInfo.vad_model_path,
        });
      } else {
        await invoke('plugin:whisper-core|load_vad_model', { modelPath: null });
      }

      // 3. Load Diarization if enabled
      if (diarizationEnabled && modelsInfo?.diarization_model_dir) {
        await invoke('plugin:whisper-core|load_diarization_model', {
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      await invoke('plugin:whisper-core|start_stream', {
        context: {
          language: langParam,
      ...
      if (audioSource === 'sck' || audioSource === 'both') {
        ...
        await invoke('plugin:whisper-core|start_sck_capture', {
  ...
  const stopRecording = useCallback(async () => {
    ...
    try {
      ...
      if (audioSource === 'sck' || audioSource === 'both') {
        await invoke('plugin:whisper-core|stop_sck_capture');
      }
      await invoke('plugin:whisper-core|stop_stream');
  ...
  const startDiskRecording = useCallback(async () => {
    ...
    try {
      ...
      await invoke('plugin:whisper-core|start_recording', {});
      wavStarted = true;
      ...
      if (wavStarted) {
        try { await invoke('plugin:whisper-core|stop_recording'); } catch { /* ignore */ }
      }
      if (streamStarted) {
        try { await invoke('plugin:whisper-core|stop_stream'); } catch { /* ignore */ }
      }
  ...
  const stopDiskRecording = useCallback(async () => {
    try {
      ...
      await invoke('plugin:whisper-core|stop_stream');
      const info = await invoke<RecordingInfo>('plugin:whisper-core|stop_recording');
  ...
  const createTranscript = useCallback(async () => {
    ...
    try {
      const langParam = language === 'auto' ? null : language;
      const result = await invoke<RecordingTranscript>('plugin:whisper-core|transcribe_recording', {
  ...
  useEffect(() => {
    ...
    async function setupListeners() {
      try {
        const uPartial = await listen<PartialResultPayload>('whisper-core-partial-result', event => {
          if (!isMounted) return;
        ...
        const uDiarized = await listen<DiarizedResultPayload>('whisper-core-diarized-result', event => {
          if (!isMounted) return;
        ...
        const uProgress = await listen<{ percent: number }>('whisper-core-download-progress', event => {
          if (!isMounted) return;
