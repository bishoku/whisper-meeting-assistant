...
pub fn run_worker<R: Runtime>(
    ...
    let _ = app_handle.emit("whisper-core-stream-stopped", ());
...
fn process_utterance<R: Runtime>(
    ...
    let partial_cb = std::sync::Arc::new(move |partial: crate::state::PartialResultPayload| {
        let _ = handle_clone.emit("whisper-core-partial-result", &partial);
    });
    ...
    let _ = app_handle.emit("whisper-core-diarized-result", &d_payload);
}
